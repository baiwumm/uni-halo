  ...
  <view class="app-page">
    ...
    <block v-else>
      ...
      <view class="content ml-24 mr-24">
        ...
        <!-- 投票 -->
        <view
          v-if="!calcAuditModeEnabled && result._voteIds.length !== 0"
        ...
        <!-- 版权声明 -->
        <view
          v-if="postDetailConfig && postDetailConfig.copyrightEnabled"
...
export default {
  ...
  methods: {
    ...
    fnHandleSetFlotButtonItems(configs) {
      const actions = [
        ...
        },
        {
          ...
          color: upvote.has("post", this.result?.metadata?.name)
    ...
    restrictReadCheck() {
      ...
      this.$httpApi.v2
        .requestRestrictReadCheck(
          this.result?.metadata?.annotations?.restrictReadEnable,
          this.restrictReadInputCode,
          this.result?.metadata?.name
        )
        .then((res) => {
          if (res.code === 200) {
